...
class style_transfer_module():
    ...
    def get_text_condition(self, text):
        ...
        denoise_kwargs = {
            'encoder_hidden_states': jt.concat(text_cond) # torch.cat(text_cond)
        }
        ...
        for t in tqdm(self.scheduler.timesteps):
            ...
            with jt.no_grad(): # torch.no_grad():
                ...
                if 'encoder_hidden_states' in denoise_kwargs.keys():
                    ...
                    input = jt.concat([input] * bs) # torch.cat([input] * bs)
    ...
    def invert_process(self, input, denoise_kwargs):
        ...
        timesteps = self.scheduler.timesteps.numpy().tolist()
        ...
        with jt.no_grad(): # torch.no_grad():
            for i in tqdm(range(0, num_inference_steps)):
                ...
                # t = timesteps[i]
                t = jt.Var(timesteps[num_inference_steps-i-1])
                ...
                if 'encoder_hidden_states' in denoise_kwargs.keys():
                    bs = denoise_kwargs['encoder_hidden_states'].shape[0]
                    cur_latent = jt.concat([cur_latent] * bs) # torch.cat([cur_latent] * bs)
...
if __name__ == "__main__":
    ...
    # Read style image from a folder
    sty_no = "{:0>2d}".format(cfg.sty_no)
    images_path = cfg.sty_folder + "/" + sty_no + "/images"
    sty_files = [os.path.join(images_path, pic) for pic in os.listdir(images_path) if ".png" in pic]
    
    print("Load style images: ", sty_files)

    sty_file_idx = np.random.randint(0, len(sty_files))
    print("Use style image: ", sty_files[sty_file_idx])
    style_image = cv2.imread(sty_files[sty_file_idx])[:, :, ::-1]
    ...
    device = "cuda"
    dtype = jt.float16 # torch.float16
    in_c = 4
    ...
    if cfg.without_init_adain:
        ...
        # jittor adaption:
        content_latent_mean = content_latent.mean(dim=3, keepdim=True).mean(dim=2, keepdim=True)
        style_latent_mean = style_latent.mean(dim=3, keepdim=True).mean(dim=2, keepdim=True)

        # jittor has std, but no dim args
        def std(x):
            N, C, H, W = x.shape
            x1 = x.view(N, C, -1)
            ret = jt.empty((N, C, 1, 1))
            for i in range(N):
                for j in range(C):
                    ret[i, j, 0, 0] = jt.std(x1[i, j])
            return ret
    
        content_latent_std = std(content_latent)
        style_latent_std = std(style_latent)
        
        latent_cs = (content_latent - content_latent_mean) / (content_latent_std + 1e-4) * style_latent_std + style_latent_mean
    ...
    save_image(image_last, os.path.join(save_dir, cfg.result_fn))
