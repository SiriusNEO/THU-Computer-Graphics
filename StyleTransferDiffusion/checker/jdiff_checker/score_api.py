...
import argparse
from dino_api import dino_style_similarity
...
def get_score(upload_path, result_path):
    ...
    final_score = (result['dino_score']*0.2 + result['hist_score']*0.4 + (1-result['lpip_score'])*0.4) * (result['clip_r_score']*0.5+(20-min(result['fid_score'],20))/20*0.4+result['clip_iqa_score']*0.1)
